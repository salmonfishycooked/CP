...
public:
    struct Tag {
        ...
        Tag() : mul(1), plus(0) {}
        Tag(i64 _mul, i64 _plus) : mul(_mul), plus(_plus) {}
...
private:
    ...
    Info combineInfoAndTag(int id1, const Tag &tag2) {
        ...
        info.v = (info.v * tag2.mul + tag2.plus * (r - l + 1)) % m;
    ...
    Tag combineTagAndTag(int id1, const Tag &tag2) {
        ...
        Tag tag;
        tag.mul = tag1.mul * tag2.mul % m;
        tag.plus = (tag1.plus * tag2.mul + tag2.plus) % m;
...
void solve() {
    ...
    while (q--) {
        ...
        if (op == 1) {
            std::cin >> k;
            tree.modify(1, x, y, {k, 0});
        } else if (op == 2) {
            std::cin >> k;
            tree.modify(1, x, y, {1, k});
        } else {
